              ...
              {it.type === 'anchor' ? (
                ...
                <div className="rounded-xl border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 p-4">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    ...
                    <div className="text-sm text-slate-700 dark:text-slate-300">{toISO(it.arrival)} → {toISO(it.departure)}</div>
                  </div>
                  <div className="mt-3 grid sm:grid-cols-2 gap-x-6 gap-y-2">
                    <div className="text-sm"><span className="font-medium">Accommodation:</span> {it.accommodation || '—'}</div>
                    <div className="text-sm"><span className="font-medium">Vaccines:</span> {it.vaccines?.length ? it.vaccines.join(', ') : '—'}</div>
                    <div className="text-sm"><span className="font-medium">Malaria prophylaxis:</span> {it.malaria?.took ? `${it.malaria.drug}${it.malaria.adherence ? ` (${it.malaria.adherence})` : ''}` : 'No'}</div>
                ...
                <div className="mt-2 ml-1 flex flex-wrap items-center gap-2">
                  {between.map((l) => (
                    <span key={l.id} className="inline-flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                      ...
                      {l.start ? ` (${toISO(l.start)}` : ''}{l.end ? ` → ${toISO(l.end)})` : (l.start ? ')' : '')}
                    </span>
                  ))}
                </div>
              )}
...
function TimelineVertical({ stops, layovers }) {
  ...
  // For association: layovers whose start falls between item A and B
  const layoversBetween = (a, b) => {
    const aEnd = parseDate(a?.departure || a?.arrival);
    const bStart = parseDate(b?.arrival || b?.departure);
    if (!aEnd || !bStart) return [];
