...
// Travel History Generator — v45 (Clinical Polish: Smart Layovers & Natural Summary)
// Changes:
// - UI: Layover Card now defaults to single-day input (writes to start & end).
// - SUMMARY: Natural language generation ("Transit via..." instead of lists).
// - LOGIC: Dynamic TravelMap import retained.
// - UX: Added visual warnings for overlapping dates.
...
import { format, differenceInDays, addDays, min, max, isValid, parseISO, isSameDay, isWithinInterval } from 'date-fns';
...
// Exposure Categories (Same as before)
...
// Smooth Reveal, Date Picker, Comboboxes (Same as before)
...
function SearchableSelect({ value, onChange, options, placeholder, allowCustom = false }) {
  ...
  return (
    <Combobox value={value} onChange={onChange} nullable>
      <div className="relative mt-1">
        <div className={CONTAINER_BASE}>
          <ComboboxInput className={INPUT_BASE} displayValue={(item) => item || ''} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder} />
          <ComboboxButton className="absolute inset-y-0 right-0 flex items-center pr-2"><Icons.ChevronUpDown aria-hidden="true" /></ComboboxButton>
        </div>
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0" afterLeave={() => setQuery('')}>
          <ComboboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg bg-white dark:bg-slate-900 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
            {filteredOptions.length === 0 && query !== '' ? (
              allowCustom ? (
                <ComboboxOption className={({ active }) => clsx('relative cursor-pointer select-none py-2 pl-4 pr-4', active ? 'bg-[hsl(var(--brand))] text-white' : 'text-slate-900 dark:text-slate-100')} value={query}><div className="flex items-center gap-2"><Icons.Plus className="h-4 w-4" /><span>Use "{query}"</span></div></ComboboxOption>
              ...
              filteredOptions.map((opt, idx) => {
                ...
                return (
                  <ComboboxOption key={key} className={({ active }) => clsx('relative cursor-default select-none py-2 pl-10 pr-4', active ? 'bg-[hsl(var(--brand))] text-white' : 'text-slate-900 dark:text-slate-100')} value={label}>
                    {({ selected, active }) => (
                      <><span className={clsx('block truncate', selected ? 'font-medium' : 'font-normal')}>{label}</span>{selected ? (<span className={clsx('absolute inset-y-0 left-0 flex items-center pl-3', active ? 'text-white' : 'text-[hsl(var(--brand))]')}><Icons.Check aria-hidden="true" /></span>) : null}</>
                    )}
                  </ComboboxOption>
                );
              })
...
function MultiSelectTags({ value = [], onChange, options, placeholder }) {
  ...
  const filteredOptions = useMemo(() => {
    const q = normalize(query);
    return query === '' ? options.filter(opt => !value.includes(opt)) : options.filter((opt) => { return normalize(opt).includes(q) && !value.includes(opt); });
  }, [query, options, value]);
        ...
        <div className={clsx(CONTAINER_BASE, "flex flex-wrap items-center gap-1.5 p-1.5 min-h-[42px]")}>
          {value.map((tag) => (
            <span key={tag} className="inline-flex items-center gap-1 rounded bg-[hsl(var(--brand))]/10 border border-[hsl(var(--brand))]/20 px-2 py-0.5 text-xs font-medium text-[hsl(var(--brand))] dark:text-[hsl(var(--accent))]">
              {tag}<button type="button" className="group relative -mr-1 h-3.5 w-3.5 rounded-sm hover:bg-[hsl(var(--brand))]/20" onClick={(e) => { e.stopPropagation(); removeTag(tag); }}><Icons.X className="h-3 w-3" /></button>
            </span>
          ))}
          <ComboboxInput className="min-w-[120px] flex-1 border-none bg-transparent py-1 pl-1 text-sm leading-5 text-slate-900 focus:ring-0 dark:text-slate-100 placeholder:text-slate-400" displayValue={() => query} onChange={(event) => setQuery(event.target.value)} placeholder={value.length === 0 ? placeholder : ""} />
        </div>
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0" afterLeave={() => setQuery('')}>
          <ComboboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg bg-white dark:bg-slate-900 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
            {filteredOptions.length === 0 && query !== '' ? (
              <ComboboxOption className={({ active }) => clsx('relative cursor-pointer select-none py-2 pl-4 pr-4', active ? 'bg-[hsl(var(--brand))] text-white' : 'text-slate-900 dark:text-slate-100')} value={query}><div className="flex items-center gap-2"><Icons.Plus className="h-4 w-4" /><span>Add "{query}"</span></div></ComboboxOption>
            ) : (
              filteredOptions.map((opt, idx) => (
                <ComboboxOption key={idx} className={({ active }) => clsx('relative cursor-default select-none py-2 pl-4 pr-4', active ? 'bg-[hsl(var(--brand))] text-white' : 'text-slate-900 dark:text-slate-100')} value={opt}><span className="block truncate font-normal">{opt}</span></ComboboxOption>
              ))
            )}
          </ComboboxOptions>
        </Transition>
      ...
      <div className="relative mt-1">
        ...
        <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
          <ListboxOptions className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-lg bg-white dark:bg-slate-900 py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
            {options.map((opt, idx) => (
              <ListboxOption key={idx} className={({ active }) => clsx('relative cursor-default select-none py-2 pl-10 pr-4', active ? 'bg-[hsl(var(--brand))] text-white' : 'text-slate-900 dark:text-slate-100')} value={opt}>
                {({ selected, active }) => (
                  <><span className={clsx('block truncate', selected ? 'font-medium' : 'font-normal')}>{opt}</span>{selected ? (<span className={clsx('absolute inset-y-0 left-0 flex items-center pl-3', active ? 'text-white' : 'text-[hsl(var(--brand))]')}><Icons.Check aria-hidden="true" /></span>) : null}</>
                )}
              </ListboxOption>
            ))}
          </ListboxOptions>
...
const emptyStop = () => ({
  ...
  exposures: { mosquito: 'unknown', tick: 'unknown', vectorOtherEnabled: 'unknown', freshwater: 'unknown', cavesMines: 'unknown', ruralForest: 'unknown', hikingWoodlands: 'unknown', animalContact: 'unknown', animalBiteScratch: 'unknown', batsRodents: 'unknown', bushmeat: 'unknown', needlesTattoos: 'unknown', safariWildlife: 'unknown', streetFood: 'unknown', untreatedWater: 'unknown', undercookedFood: 'unknown', undercookedSeafood: 'unknown', unpasteurisedMilk: 'unknown', funerals: 'unknown', sickContacts: 'unknown', healthcareFacility: 'unknown', prison: 'unknown', refugeeCamp: 'unknown', unprotectedSex: 'unknown', positiveDetails: '', otherText: '' },
});
...
const emptyTrip = () => ({
  id: uid(), purpose: '', originCountry: 'United Kingdom', originCity: 'Manchester',
  vaccines: { status: 'unknown', details: [] }, malaria: { indication: 'Not indicated', drug: 'None', adherence: '' },
  companions: { group: 'Alone', otherText: '', companionsWell: 'unknown', companionsUnwellDetails: '' }, stops: [emptyStop()], layovers: [],
});
const initialState = { trips: [emptyTrip()], pastTravels: [] };

// ===== Shared chronology builder (Logic remains, Text generation updated below) =====
...
export default function TravelHistoryGeneratorPage() {
  ...
  useEffect(() => {
    ...
    state.trips.forEach((trip, tIdx) => {
      ...
      // 2. Overlapping Layover check
      trip.layovers.forEach((l) => {
        if (!l.start) return;
        const lStart = parseDate(l.start);
        const lEnd = l.end ? parseDate(l.end) : lStart;
        
        trip.stops.forEach((s) => {
           if (!s.arrival || !s.departure) return;
           const sStart = parseDate(s.arrival);
           const sEnd = parseDate(s.departure);
           // If layover overlaps entirely with a stop (e.g. they said they were in Dubai while they said they were in Thailand)
           if (lStart >= sStart && lEnd <= sEnd) {
              list.push({ level: 'warning', msg: `Warning: Layover in ${l.city||l.country} overlaps with stay in ${s.country}.` });
              layIds.add(l.id);
           }
        });
      });
            ...
            <div key={pt.id} ref={setItemRef(pt.id)} className="grid gap-4 sm:grid-cols-12 items-start p-4 rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20">
              ...
              <div className="sm:col-span-2">
                ...
                <div className={TEXT_INPUT_CLASS}>
                  <input type="text" placeholder="e.g. 2012" className={INPUT_BASE} value={pt.year} onChange={(e) => updatePastTravel(pt.id, { year: e.target.value })} />
                </div>
              </div>
              <div className="sm:col-span-5">
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Details</label>
                <textarea rows={1} placeholder="Describe details..." className={clsx(TEXTAREA_CLASS, "min-h-[42px]")} value={pt.details} onChange={(e) => updatePastTravel(pt.id, { details: e.target.value })} />
              </div>
              <div className="sm:col-span-1 flex justify-end pt-6">
                <button type="button" onClick={() => removePastTravel(pt.id)} className="text-slate-400 hover:text-rose-500 transition p-2"><Icons.Trash className="w-5 h-5" /></button>
              </div>
            </div>
          ))}
...
// ... [TripCard, StopCard, ExposureTagSystem remain unchanged from previous polished version] ...
// To save space, assuming those are identical. 
// RE-INSERTING TripCard and others below for completeness to ensure no copy-paste errors.

function TripCard({ trip, index, totalTrips, updateTrip, updateStop, addStop, removeStop, addLayover, updateLayover, removeLayover, removeTrip, highlight, setItemRef, innerRef }) {
  ...
  const setVaccines = (patch) => { const next = { ...trip.vaccines, ...patch }; if (next.status !== 'Taken') { next.details = []; } updateTrip(trip.id, { vaccines: next }); };
  const updateCompanions = (patch) => { updateTrip(trip.id, { companions: { ...trip.companions, ...patch } }); };
  ...
  return (
    <div ref={innerRef} className="rounded-xl border-2 border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950 p-6">
      <div className="flex items-start justify-between gap-3">
        <h2 className={SECTION_HEADING}>{totalTrips > 1 ? `Trip ${index + 1}` : "Trip details"}</h2>
        <div className="flex gap-2">
      ...
      <div className="mt-6">
        ...
        <div className="mt-2 grid sm:grid-cols-2 gap-4">
          <div><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">Country</label><SearchableSelect value={trip.originCountry} onChange={(val) => updateTrip(trip.id, { originCountry: val, originCity: '' })} options={CSC_COUNTRIES.map(c => c.name)} placeholder="Select country" /></div>
          <div className="w-full"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">City</label><SearchableSelect value={trip.originCity} onChange={(val) => updateTrip(trip.id, { originCity: val })} options={originCityNames} placeholder="Search city" allowCustom={true} /></div>
        </div>
      </div>
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 p-4 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/30">
        <div><label className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-1">What was the reason for travel?</label><div className={TEXT_INPUT_CLASS}><input type="text" placeholder="Work, VFR, tourism, etc." className={INPUT_BASE} value={trip.purpose} onChange={(e) => updateTrip(trip.id, { purpose: e.target.value })} /></div></div>
        <div>
          <label className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-2">Who did they travel with?</label>
          <div className="flex flex-wrap gap-2">{COMPANION_GROUPS.map((opt) => (<button key={opt} type="button" onClick={() => { const next = { group: opt }; if (opt === 'Alone') { next.companionsWell = 'unknown'; next.companionsUnwellDetails = ''; next.otherText = ''; } updateCompanions(next); }} className={clsx("px-3 py-1.5 text-xs font-medium rounded-lg border transition", trip.companions.group === opt ? "bg-[hsl(var(--brand))] text-white border-transparent" : "bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-[hsl(var(--brand))]")}>{opt}</button>))}</div>
          <SmoothReveal show={trip.companions.group === 'Other'}><div className="mt-2"><div className={TEXT_INPUT_CLASS}><input type="text" className={INPUT_BASE} placeholder="Describe companions..." value={trip.companions.otherText} onChange={(e) => updateCompanions({ otherText: e.target.value })} /></div></div></SmoothReveal>
          <SmoothReveal show={trip.companions.group !== 'Alone'}><div className="mt-2 grid gap-2"><div><label className="block text-xs font-medium text-slate-500 mb-1">Are they well?</label><div className="flex gap-2">{COMPANION_WELL_OPTIONS.map((opt) => { const val = opt.toLowerCase(); return (<button key={val} type="button" onClick={() => updateCompanions({ companionsWell: val, companionsUnwellDetails: val === 'no' ? trip.companions.companionsUnwellDetails : '' })} className={clsx("px-2 py-1 text-xs font-medium rounded border transition", trip.companions.companionsWell === val ? "bg-[hsl(var(--brand))] text-white border-transparent" : "bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-700 text-slate-500 hover:border-slate-400")}>{opt}</button>); })}</div></div><SmoothReveal show={trip.companions.companionsWell === 'no'}><div><div className={TEXT_INPUT_CLASS}><input type="text" placeholder="Details (symptoms, etc.)" className={INPUT_BASE} value={trip.companions.companionsUnwellDetails} onChange={(e) => updateCompanions({ companionsUnwellDetails: e.target.value })} /></div></div></SmoothReveal></div></SmoothReveal>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">{VACCINE_STATUS_OPTIONS.map(opt => (<button key={opt} type="button" onClick={() => setVaccines({ status: opt })} className={clsx("px-3 py-1.5 text-xs font-medium rounded-lg border transition", (trip.vaccines?.status || 'unknown') === opt ? "bg-[hsl(var(--brand))] text-white border-transparent" : "bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-[hsl(var(--brand))]")}>{opt}</button>))}</div>
          <SmoothReveal show={trip.vaccines?.status === 'Taken'}><div><label className="block text-xs font-medium text-slate-500 mb-1">Details (Select multiple or type custom)</label><MultiSelectTags value={trip.vaccines.details || []} onChange={(val) => setVaccines({ details: val })} options={VACCINE_SUGGESTIONS} placeholder="Select or type..." /></div></SmoothReveal>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-2">{MALARIA_STATUS_OPTIONS.map(opt => (<button key={opt} type="button" onClick={() => setMalaria({ indication: opt })} className={clsx("px-3 py-1.5 text-xs font-medium rounded-lg border transition", trip.malaria.indication === opt ? "bg-[hsl(var(--brand))] text-white border-transparent" : "bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 hover:border-[hsl(var(--brand))]")}>{opt}</button>))}</div>
          <SmoothReveal show={trip.malaria.indication === 'Taken'}><div className="grid grid-cols-2 gap-3 pt-1"><div><label className="block text-xs font-medium text-slate-500 mb-1">Drug</label><SimpleSelect value={trip.malaria.drug} onChange={(val) => setMalaria({ drug: val })} options={MALARIA_DRUGS} /></div><div><label className="block text-xs font-medium text-slate-500 mb-1">Adherence</label><SimpleSelect value={trip.malaria.adherence} onChange={(val) => setMalaria({ adherence: val })} options={ADHERENCE_OPTIONS} /></div></div></SmoothReveal>
        </div>
      </div>
      <div className="mt-6 space-y-6">{trip.stops.map((stop, sIdx) => (<StopCard key={stop.id} innerRef={setItemRef(stop.id)} stop={stop} index={sIdx} totalStops={trip.stops.length} onChange={(patch) => updateStop(trip.id, stop.id, patch)} onRemove={() => removeStop(trip.id, stop.id)} highlighted={highlight.stopIds.has(stop.id)} />))}</div>
...
function StopCard({ stop, index, totalStops, onChange, onRemove, innerRef, highlighted }) {
  ...
  const normalizedCities = (stop.cities || []).map((c) => typeof c === 'string' ? { name: c || '', arrival: '', departure: '' } : { name: c?.name || '', arrival: c?.arrival || '', departure: c?.departure || '' });
  const countryISO2 = useMemo(() => { const name = (stop.country || "").trim().toLowerCase(); if (!name) return null; const match = Country.getAllCountries().find((c) => c.name.trim().toLowerCase() === name); return match?.isoCode || null; }, [stop.country]);
  ...
  return (
    <div ref={innerRef} className={classNames("rounded-lg border p-4", highlighted ? "border-amber-400 dark:border-amber-600 bg-amber-50/40 dark:bg-amber-900/10" : "border-slate-200 dark:border-slate-800")}>
      <div className="flex items-start justify-between gap-3"><h3 className="text-base font-semibold text-slate-900 dark:text-slate-100">{totalStops > 1 ? `Destination ${index + 1}` : "Destination"}</h3><button type="button" onClick={onRemove} className={LINKISH_SECONDARY}>Remove destination</button></div>
      <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="w-full"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">Country</label><SearchableSelect value={stop.country} onChange={(val) => onChange({ country: val })} options={CSC_COUNTRIES.map(c => c.name)} placeholder="Select country" /></div>
      ...
      <div className="mt-4">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-1"><div><label className="block text-sm text-slate-600 dark:text-slate-300">City</label></div><div><label className="block text-sm text-slate-600 dark:text-slate-300">Arrival</label></div><div><label className="block text-sm text-slate-600 dark:text-slate-300">Departure</label></div></div>
        <div className="space-y-2">{normalizedCities.map((row, i) => (<div key={i} className="grid sm:grid-cols-3 lg:grid-cols-4 gap-4"><div className="w-full"><SearchableSelect value={row.name} onChange={(val) => setCityName(i, val)} options={cityOptions.map(c => c.name)} placeholder="Search city" allowCustom={true} /></div><ResponsiveDatePicker value={row.arrival} onChange={(val) => setCityArrival(i, val)} /><ResponsiveDatePicker value={row.departure} onChange={(val) => setCityDeparture(i, val)} /><div className="flex"><button type="button" onClick={() => removeCity(i)} className="w-full sm:w-auto rounded-lg px-3 py-2 text-xs border-2 border-slate-300 dark:border-slate-700 hover:border-[hsl(var(--brand))] dark:hover:border-[hsl(var(--accent))] transition">Remove</button></div></div>))}<button type="button" onClick={addCity} className="rounded-lg px-3 py-1.5 text-xs font-medium border border-slate-300 dark:border-slate-700 hover:border-[hsl(var(--brand))] dark:hover:border-[hsl(var(--accent))] transition text-slate-600 dark:text-slate-400">+ Add another city</button></div>
      </div>
      <div className="mt-4"><label className="block text-sm font-semibold text-slate-800 dark:text-slate-200 mb-2">What sort of accommodation did they stay in?</label><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">{ACCOMMODATION_OPTIONS.map((opt) => { const checked = (stop.accommodations || []).includes(opt); const id = `${stop.id}-accom-${opt.replace(/\s+/g, '-').toLowerCase()}`; return (<label key={opt} htmlFor={id} className="flex items-start gap-2 py-1 text-sm text-slate-700 dark:text-slate-300"><input id={id} type="checkbox" className="h-4 w-4 mt-0.5 rounded border-slate-300 dark:border-slate-700 text-[hsl(var(--brand))] focus:ring-[hsl(var(--brand))]" checked={checked} onChange={() => toggleAccommodation(opt)} /><span>{opt}</span></label>); })}</div>{(stop.accommodations || []).includes('Other') && (<div className="mt-2"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">Other (describe)</label><div className={TEXT_INPUT_CLASS}><input type="text" className={INPUT_BASE} value={stop.accommodationOther} onChange={(e) => onChange({ accommodationOther: e.target.value })} /></div></div>)}</div>
      <div className="mt-6 border-t border-slate-200 dark:border-slate-800 pt-6"><h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 mb-4">Did they have any of the following exposures or activities?</h4><ExposureTagSystem exposures={exp} onChange={(newExp) => onChange({ exposures: newExp })} /></div>
    </div>
  );
}
...
function ExposureTagSystem({ exposures, onChange }) {
  ...
  const markRestAsNo = () => { const patch = {}; EXPOSURE_CATEGORIES.forEach(cat => { cat.items.forEach(item => { const key = item.key; const current = exposures[key] || 'unknown'; if (current === 'unknown') { patch[key] = 'no'; } }); }); onChange({ ...exposures, ...patch }); };
  const hasPositive = EXPOSURE_CATEGORIES.some(cat => cat.items.some(item => exposures[item.key] === 'yes' || exposures[item.key] === true));

  return (
    <div className="space-y-6">
      <div className="space-y-6">{EXPOSURE_CATEGORIES.map((cat) => (<div key={cat.title}><div className="text-xs font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-2 border-b border-slate-100 dark:border-slate-800 pb-1">{cat.title}</div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">{cat.items.map((item) => { const status = exposures[item.key] || 'unknown'; const isYes = status === 'yes' || status === true; const isNo = status === 'no' || status === false; return (<div key={item.key} className="flex items-center justify-between p-2 rounded-lg border bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 transition-all"><span className="text-xs font-medium mr-2 truncate text-slate-700 dark:text-slate-300" title={item.label}>{item.label}</span><div className="flex items-center gap-1 shrink-0"><button type="button" onClick={() => setItem(item.key, 'yes')} className={clsx("px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded border transition-colors", isYes ? "bg-[hsl(var(--brand))] text-white border-transparent" : "bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-500 hover:border-slate-400 hover:text-slate-700")}>Yes</button><button type="button" onClick={() => setItem(item.key, 'no')} className={clsx("px-2 py-1 text-[10px] font-bold uppercase tracking-wider rounded border transition-colors", isNo ? "bg-slate-600 text-white border-transparent" : "bg-white dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-500 hover:border-slate-400 hover:text-slate-700")}>No</button></div></div>); })}</div></div>))}</div>
      <div className="flex justify-end pt-2"><button type="button" onClick={markRestAsNo} className="text-xs font-medium text-[hsl(var(--brand))] hover:underline underline-offset-4 decoration-2">Mark remaining as 'No'</button></div>
      <SmoothReveal show={hasPositive}><div className="space-y-3 pt-2 border-t border-slate-100 dark:border-slate-800"><label className="block text-sm font-semibold text-slate-900 dark:text-slate-100">Kindly provide more detail about positive exposures</label><textarea rows={3} className={TEXTAREA_CLASS} value={exposures.positiveDetails || ''} onChange={(e) => onChange({ ...exposures, positiveDetails: e.target.value })} /></div></SmoothReveal>
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1 font-medium">Any other trip details or exposures</label><textarea rows={2} className={TEXTAREA_CLASS} value={exposures.otherText} onChange={(e) => onChange({ ...exposures, otherText: e.target.value })} /></div>
    </div>
  );
}

// ==== UPDATED LAYOVER CARD (The Fix) ====
function LayoverCard({ layover, onChange, onRemove, innerRef, highlighted }) {
  ...
  // Logic: Is it multi-day?
  const isMultiDay = layover.start && layover.end && layover.start !== layover.end;
  const [showEnd, setShowEnd] = useState(isMultiDay);

  const handleDateChange = (val) => {
    // If single day mode, set both start and end to the same date
    if (!showEnd) {
      onChange({ start: val, end: val });
    } else {
      onChange({ start: val });
    }
  };
  ...
  const toggleMultiDay = () => {
    const nextShow = !showEnd;
    setShowEnd(nextShow);
    if (!nextShow) {
      // Collapsing: Reset end to match start
      onChange({ end: layover.start });
    }
  };

  return (
    <div ref={innerRef} className={classNames("rounded-lg border p-4 transition-colors duration-300", highlighted ? "border-amber-400 dark:border-amber-600 bg-amber-50/40 dark:bg-amber-900/10" : "border-slate-200 dark:border-slate-800")}>
      <div className="flex items-start justify-between gap-3">
        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
          Layover {highlighted && <span className="text-[10px] font-bold text-amber-600 uppercase bg-amber-100 px-1.5 py-0.5 rounded">Overlap Warning</span>}
        </h4>
        <button type="button" onClick={onRemove} className={LINKISH_SECONDARY}>Remove layover</button>
      </div>
      <div className="mt-4 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="w-full"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">Country</label><SearchableSelect value={layover.country} onChange={(val) => { onChange({ country: val, city: "" }); }} options={CSC_COUNTRIES.map(c => c.name)} placeholder="Select country" /></div>
        <div className="w-full"><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">City</label><SearchableSelect value={layover.city} onChange={(val) => onChange({ city: val })} options={cityOptions.map(c => c.name)} placeholder="Search city" allowCustom={true} /></div>
        
        {/* Smart Date Input */}
        <div>
           <div className="flex items-center justify-between mb-1">
             <label className="block text-sm text-slate-600 dark:text-slate-300">{showEnd ? "Start Date" : "Date of Transit"}</label>
             <button type="button" onClick={toggleMultiDay} className="text-[10px] text-[hsl(var(--brand))] hover:underline underline-offset-2">
               {showEnd ? "Single day?" : "Multi-day?"}
             </button>
           </div>
           <ResponsiveDatePicker value={layover.start} onChange={handleDateChange} />
        </div>
      ...
      <div className="mt-4 grid sm:grid-cols-3 gap-4">
        <div><label className="block text-sm text-slate-600 dark:text-slate-300 mb-1">Did you leave the airport?</label><SimpleSelect value={layover.leftAirport} onChange={(val) => onChange({ leftAirport: val })} options={['no', 'yes']} /></div>
...
// ==== UPDATED SUMMARY ENGINE (The Fix) ====
function buildSummaryFromEvents(state, mergedEventsAllTrips) {
  ...
  for (const [tripId, events] of byTrip.entries()) {
    ...
    html.push(`<div>Country / countries travelled (See below for details): ${escapeHtml(countriesCsv)}</div>`); text.push(`Country / countries travelled (See below for details): ${countriesCsv}`);
    ...
    // Malaria / Vaccines logic (same as before, condensed for space)
    { const m = tripObj.malaria || { indication: "Not indicated", drug: "None", adherence: "" }; let malariaText = m.indication === "Unsure" ? "Unsure" : m.indication === "Taken" ? `Taken — ${m.drug} (Adherence: ${m.adherence})` : "Not taken"; html.push(`<div>Malaria prophylaxis: ${escapeHtml(malariaText)}</div>`); text.push(`Malaria prophylaxis: ${malariaText}`); }
    { const v = tripObj.vaccines || { status: 'unknown', details: [] }; let vaccineText = v.status === 'Taken' ? `Taken: ${v.details?.join(', ') || 'No details'}` : v.status; html.push(`<div>Pre-travel vaccinations: ${escapeHtml(vaccineText)}</div>`); text.push(`Pre-travel vaccinations: ${vaccineText}`); }
    { const cmp = tripObj.companions || {}; const groupStr = cmp.group === "Other" ? cmp.otherText || "Other" : cmp.group || "—"; html.push(`<div>Travelled with: ${escapeHtml(groupStr)}</div>`); text.push(`Travelled with: ${groupStr}`); const wellStr = cmp.companionsWell === "yes" ? "Yes" : cmp.companionsWell === "no" ? `No — ${cmp.companionsUnwellDetails||''}` : "Unknown"; html.push(`<div>Are they well: ${escapeHtml(wellStr)}</div>`); text.push(`Are they well: ${wellStr}`); }

    // Helper for formatting transit text naturally
    const fmtTransit = (l) => { 
      const place = `${(l.city ? `${l.city}, ` : "") + (l.country || "Unknown Location")}`.trim(); 
      const isMulti = l.start && l.end && l.start !== l.end;
      const dates = isMulti ? `(${formatDMY(l.start)}–${formatDMY(l.end)})` : `(${formatDMY(l.start)})`;
      const act = l.leftAirport === "yes" && (l.activitiesText || "").trim() ? ` — Left airport: ${l.activitiesText.trim()}` : ""; 
      const line = `Transit via ${place} ${dates}${act}`; 
      return { html: `<em>${escapeHtml(line)}</em>`, text: line }; 
    };
    ...
    events.forEach((ev) => {
      ...
      // 1. Pre-Stop Transits
      const beforeList = (layoversByStop.get(s.id)?.before || []).map(fmtTransit);
      beforeList.forEach(t => { html.push(`<div>${t.html}</div>`); text.push(t.text); });

      // 2. The Stop Header
      html.push(`<div style="height:12px"></div>`); text.push("");
      const countryDates = `${formatDMY(s.arrival)} to ${formatDMY(s.departure)}`;
      html.push(`<p><strong>${escapeHtml(s.country)} (${escapeHtml(countryDates)})</strong></p>`); text.push(`${s.country} (${countryDates})`);
      ...
      if (citiesArr.length) { 
         const lines = citiesArr.map(c => `${c.name}${c.arrival ? ` (${formatDMY(c.arrival)}-${formatDMY(c.departure)})` : ''}`);
         html.push(`<div>Cities: ${escapeHtml(lines.join(", "))}</div>`); text.push(`Cities: ${lines.join(", ")}`);
      }
      ...
      html.push(`<div>Accommodation: ${escapeHtml(accom)}</div>`); text.push(`Accommodation: ${accom}`);

      // 4. Exposures
      const { positives, negatives, otherText } = exposureBullets(s.exposures);
      if (positives.length > 0) { 
        html.push(`<div><strong>Exposures:</strong> ${escapeHtml(positives.map(p => p.label).join(", "))}</div>`); text.push(`Exposures: ${positives.map(p => p.label).join(", ")}`);
        const narrative = positives[0]?.details; 
        if (narrative) { html.push(`<div><strong>Details:</strong> ${escapeHtml(narrative)}</div>`); text.push(`Details: ${narrative}`); }
      } else {
        html.push(`<div>Exposures: None reported.</div>`); text.push(`Exposures: None reported.`);
      }
      if (otherText) { html.push(`<div>Other details: ${escapeHtml(otherText)}</div>`); text.push(`Other details: ${otherText}`); }

      // 5. Post-Stop Transits (Between or After)
      const betweenList = (layoversByStop.get(s.id)?.between || []).map(fmtTransit);
      betweenList.forEach(t => { html.push(`<div>${t.html}</div>`); text.push(t.text); });
      
      if (s.isLastInTrip) {
        const afterList = (layoversByStop.get(s.id)?.after || []).map(fmtTransit);
        afterList.forEach(t => { html.push(`<div>${t.html}</div>`); text.push(t.text); });
      }
    });
    tripIndex += 1;
  }
  ...
  if (state.pastTravels.length > 0) {
    html.push(`<div style="height:12px"></div>`); html.push(`<p><strong>Significant Past Travel</strong></p>`); text.push(""); text.push("Significant Past Travel");
    html.push('<ul>');
    state.pastTravels.forEach(pt => {
      const line = `<strong>${escapeHtml(pt.country)}</strong> (${escapeHtml(pt.year)}): ${escapeHtml(pt.details)}`;
      const txt = `${pt.country} (${pt.year}): ${pt.details}`;
      html.push(`<li>${line}</li>`); text.push(`• ${txt}`);
    });
    html.push('</ul>');
  }
...
function exposureBullets(exp) {
  ...
  const push = (label, key) => {
    let s = exp[key]; if (typeof s === 'boolean') s = s ? 'yes' : 'unknown';
    if (s === 'yes') positives.push({ label: cap(label), details: positives.length === 0 ? detailText : '' }); 
    else if (s === 'no') negatives.push(cap(label)); 
  };
  push('mosquito bites', 'mosquito'); push('tick bites', 'tick');
  if (exp.vectorOtherEnabled === 'yes' || exp.vectorOtherEnabled === true) positives.push({ label: 'Other vector', details: positives.length === 0 ? detailText : '' });
  push('swimming in fresh water', 'freshwater'); push('caves/mines', 'cavesMines'); push('rural/forest stay', 'ruralForest'); push('hiking', 'hikingWoodlands'); push('animal contact', 'animalContact'); push('animal bite/scratch', 'animalBiteScratch'); push('bats/rodents', 'batsRodents'); push('bushmeat', 'bushmeat'); push('needles/tattoos', 'needlesTattoos'); push('safari', 'safariWildlife'); push('street food', 'streetFood'); push('untreated water', 'untreatedWater'); push('undercooked food', 'undercookedFood'); push('undercooked seafood', 'undercookedSeafood'); push('unpasteurised milk', 'unpasteurisedMilk'); push('funerals', 'funerals'); push('sick contacts', 'sickContacts'); push('healthcare contact', 'healthcareFacility'); push('prison', 'prison'); push('refugee camp', 'refugeeCamp'); push('unprotected sex', 'unprotectedSex');
