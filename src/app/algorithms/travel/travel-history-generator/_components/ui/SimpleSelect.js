...
export default function SimpleSelect({ value, onChange, options = [], placeholder = 'Select...' }) {
  ...
  const BTN_BASE = "relative w-full cursor-default rounded-lg border bg-white border-slate-300 py-2.5 pl-3.5 pr-9 text-left text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand transition-colors h-10";
    ...
    <div className="relative w-full">
      <Listbox value={value} onChange={onChange}>
        <ListboxButton className={BTN_BASE}>
          <span className={clsx("block truncate", !selectedOption && !value && "text-slate-400")}>
            {displayLabel}
          </span>
